...
class Dessert {
  ...
  String get formattedAmount => '\$$amount'; // Dynamically add the dollar sign
...
class DessertDataSource extends DataTableSource {
  ...
  DessertDataSource(this.context,
      ...
      this.hasRowHeightOverrides = true,
      this.hasZebraStripes = true]) {
    desserts = _desserts;
    if (sortedByCalories) {
      sort((d) => d.comment, true);
    }
  }
  ...
  DataRow getRow(int index, [Color? color]) {
    ...
    return DataRow2.byIndex(
      ...
      onTap: hasRowTaps
          ? () => _showSnackbar(context, 'Tapped on row ${dessert.date}')
          : null,
      onDoubleTap: hasRowTaps
          ? () => _showSnackbar(context, 'Double Tapped on row ${dessert.date}')
          : null,
      onLongPress: hasRowTaps
          ? () => _showSnackbar(context, 'Long pressed on row ${dessert.date}')
          : null,
      onSecondaryTap: hasRowTaps
          ? () => _showSnackbar(context, 'Right clicked on row ${dessert.date}')
          : null,
      onSecondaryTapDown: hasRowTaps
          ? (d) =>
              _showSnackbar(context, 'Right button down on row ${dessert.date}')
          : null,
      specificRowHeight:
          hasRowHeightOverrides && dessert.amount >= 50 ? 100 : null,
      cells: [
        DataCell(Text(dessert.date)),
        DataCell(Text(dessert.merchant)),
        DataCell(Text(dessert.formattedAmount),
            onTap: () => _showSnackbar(
                context,
                'Tapped on a cell with "${dessert.formattedAmount}"',
                Colors.red)),
        DataCell(Text(dessert.status)),
        DataCell(Text(dessert.comment)),
...
class DesertsFakeWebService {
  int Function(Dessert, Dessert)? _getComparisonFunction(
    ...
    switch (column) {
      case 'name':
        return (Dessert d1, Dessert d2) => coef * d1.date.compareTo(d2.date);
      // case 'am':
      //   return (Dessert d1, Dessert d2) => coef * (d1.date - d2.amount);
      // case 'fat':
  ...
  Future<DesertsFakeWebServiceResponse> getData(int startingAt, int count,
    ...
    return Future.delayed(
      ...
      //   result = result
      //       .where((e) =>
      //           e.date >= caloriesFilter.start &&
      //           e.date <= caloriesFilter.end)
      //       .toList();
      // }
